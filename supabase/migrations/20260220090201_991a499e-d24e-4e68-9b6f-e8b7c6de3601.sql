...
BEGIN
  ...
  SELECT
    ...
    -- total_created: job cards created on this day
    COALESCE(SUM(CASE WHEN jc.created_at::date = d.dt::date THEN 1 ELSE 0 END), 0),
    ...
    COALESCE(SUM(CASE WHEN jc.created_at::date = d.dt::date AND jc.status = 'INWARDED' THEN 1 ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN jc.created_at::date = d.dt::date AND jc.status = 'IN_PROGRESS' THEN 1 ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN jc.created_at::date = d.dt::date AND jc.status = 'READY' THEN 1 ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN jc.delivered_at::date = d.dt::date THEN 1 ELSE 0 END), 0),
