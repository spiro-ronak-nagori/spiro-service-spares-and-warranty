...
BEGIN
  ...
  INSERT INTO public.report_daily_snapshot (
    ...
    avg_mttr_minutes, avg_turnaround_minutes, reopen_percent, avg_feedback_score, feedback_count,
  ...
  SELECT
    ...
    -- avg_feedback_score: average NPS score for job cards delivered on this day in this workshop
    COALESCE((
    ...
    -- feedback_count: number of NPS responses for this day/workshop
    COALESCE((
      SELECT COUNT(fres.id)
      FROM public.feedback_responses fres
      JOIN public.feedback_requests fr ON fr.id = fres.feedback_request_id
      JOIN public.feedback_form_questions fq ON fq.id = fres.question_id
      JOIN public.job_cards jc2 ON jc2.id = fres.job_card_id
